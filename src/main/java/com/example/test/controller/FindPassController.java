...
public class FindPassController {
	...
	public String checkUserInfo(@ModelAttribute("user") User user, HttpSession session, ModelMap model)
			throws SQLException {
		// 如果验证码正常，通过邮箱
		...
		if (u1 != null && Math.abs(c1.get(Calendar.MINUTE) - c2.get(Calendar.MINUTE)) < 5
	...
	public void getVerification(@ModelAttribute("user") User user, ModelMap model, HttpServletRequest request,
		...
		String message = "please input the verification:" + verification
				+ ",within five mintues,If you do not do it yourself, ignore it";
		EmailUtil.sendEmail(to, from, subject, message);
	}

	@RequestMapping(method = RequestMethod.POST, value = "/h2")
	public String resetPass(@Valid @ModelAttribute("user") User user, Errors error, ModelMap model, HttpSession session)
			throws SQLException {
		if (error.hasFieldErrors("pass") || error.hasFieldErrors("newpass")) {
			return "updatepass";
		}
		//只有新老密码一致，才会执行后面的数据库操作
		if (session.getAttribute("oldpass").equals(user.getPass())&&updatePass(user,(Integer)session.getAttribute("userid"))) {
			model.addAttribute("user", new User());
